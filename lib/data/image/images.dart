...
class Images {
  ...
  factory Images.fromMap(Map<String, dynamic> json) => new Images(
        backdrops: json["backdrops"] == null
            ? null
            : new List<Poster>.from(
                json["backdrops"].map((x) => Poster.fromMap(x))),
        posters: json["posters"] == null
            ? null
            : new List<Poster>.from(
                json["posters"].map((x) => Poster.fromMap(x))),
      );

  Map<String, dynamic> toMap() => {
        "backdrops": backdrops == null
            ? null
            : new List<dynamic>.from(backdrops!.map((x) => x.toMap())),
        "posters": posters == null
            ? null
            : new List<dynamic>.from(posters!.map((x) => x.toMap())),
      };
}
