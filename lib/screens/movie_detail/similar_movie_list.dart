...
class SimilarMovieList extends StatelessWidget {
  ...
  const SimilarMovieList({Key? key, required this.relatedMovies})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    int rowCount = relatedMovies.length % 3 == 0
        ? relatedMovies.length
        : relatedMovies.length ~/ 3 + 1;
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return CardListCell.buildCardCell(
              index, context, relatedMovies, _cardDidTap);
        },
      ),
    );
  }
