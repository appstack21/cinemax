...
class MovieDetailScreenState extends State<MovieDetailScreen>
  ...
  Widget similarMovieListComponents() {
    Widget similarComponent = Container();
  ...
  Widget buildPageViewContainer() {
    List<Poster> posters;
    if ((detail?.images?.backdrops?.length ?? 0) > 0) {
      posters = detail?.images?.backdrops ?? [];
    } else if ((detail?.images?.posters?.length ?? 0) > 0) {
